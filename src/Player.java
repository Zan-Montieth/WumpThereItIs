...
public class Player{
    ...
    private boolean[][] knowledgeOfSafe;
    ...
    Player(int inCaveSize, Node inCave[][], Cave inUpdateMap){
        ...
        knowledgeOfSafe   = new boolean[inCaveSize][inCaveSize];
    ...
    public void findGoldSafely(int x, int y){
        ...
        if(cave[x][y].isBreeze()){knowledgeOfBreeze[x][y] = true;}
        if(cave[x][y].isStench()){knowledgeOfStench[x][y] = true;}
        int[] coords = {x,y};
        xyPath.add(coords);
        numCellsVisited++;
        ...
        visited[x][y] = true;
        updateMap.printCave();
        if(!cave[x][y].isStench() && !cave[x][y].isBreeze() && !haveVisited(x,y)){
            //TODO set all adjacent to safe
            direction = 0;
            if(canMove(x,y)) {
                moveIn(x, y);
            }else{
...
//            }
//        }
//        else if(!cave[x][y].isBreeze() && !cave[x][y].isStench() && haveVisited(x,y) && fullyExplored(x,y) && !foundGold){
//            if(!fullyExplored(x,y)) {
//                turnLeft();
//                if (canMove(x, y)) {
//                    findGoldSafely(x, y);
//                } else {
...
//            findGoldSafely(prevXY[0],prevXY[1]);
